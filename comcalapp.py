...
def render_detail_view(df_all: pd.DataFrame):
    ...
    if role == "ADMIN":
        ...
        # ✅ initialize with saved DB clawbacks
...
def render_admin_dashboard(df_all: pd.DataFrame):
    ...
    st.markdown("### 💰 Payment & Clawback Summary")
    st.markdown("""
    <style>
    table.payclaw-summary {
        width: fit-content !important;
        border-collapse: collapse;
        font-size: 13px;
    }

    table.payclaw-summary th {
    ...
    table.payclaw-summary td {
        padding: 6px 10px;
        text-align: right;
        border: 1px solid #D0D5DD;
    }

    table.payclaw-summary td:first-child,
    table.payclaw-summary th:first-child {
        text-align: left;
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    st.markdown(
    summary_df.to_html(index=False, classes="payclaw-summary"),
    unsafe_allow_html=True
    )
    ...
    for rep in CRD_REPS:
        ...
        with st.expander(f"View details for {rep}"):

            # --- Payments ---
            payments = pd.DataFrame(get_payment_history(rep))
            if not payments.empty:
                st.markdown("#### Payments")
                st.dataframe(payments, use_container_width=True)

                csv_pay = payments.to_csv(index=False).encode("utf-8")
                st.download_button(
                f"Download Payments - {rep}",
                csv_pay,
                file_name=f"{rep}_payments.csv",
                mime="text/csv",
                key=f"pay_{rep}"
            )
            else:
            ...
            # --- Clawbacks ---
            clawbacks = pd.DataFrame(get_clawbacks(rep))
            if not clawbacks.empty:
                st.markdown("#### Clawbacks")
                st.dataframe(clawbacks, use_container_width=True)

                csv_claw = clawbacks.to_csv(index=False).encode("utf-8")
                st.download_button(
                f"Download Clawbacks - {rep}",
                csv_claw,
                file_name=f"{rep}_clawbacks.csv",
                mime="text/csv",
                key=f"claw_{rep}"
                )
            else:
                st.info("No clawback records")
